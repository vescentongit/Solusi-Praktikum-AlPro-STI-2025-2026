...
TabInt ConcatTable (TabInt T1, TabInt T2){
    if (IsEmpty(T2)) return T1;
    
    IdxType start = T1.Neff + 1; // selalu dimulai dari slot setelah T1
    IdxType startT2 = IdxMin;
    T1.Neff += T2.Neff;

    for (int i = start; i <= T1.Neff; i++){
