...
Polynomial AddPolynomial(Polynomial P1, Polynomial P2){
    Polynomial new;
    int maxDeg;
    if (P1.degree > P2.degree) maxDeg = P1.degree;
    else maxDeg = P2.degree;

    CreatePolynomial(&new, maxDeg);

    for (int i = 0; i <= maxDeg; i++) {
        int c1,c2;

        if (i <= P1.degree) c1 = P1.coef[i];
        else c1 = 0;
        
        if (i <= P2.degree) c2 = P2.coef[i];
        else c2 = 0;

        new.coef[i] = c1 + c2;
    }

    while (new.degree > 0 && new.coef[new.degree] == 0) {
        new.degree--;
    }
...
Polynomial MultiplyPolynomial(Polynomial P1, Polynomial P2){
    ...
    CreatePolynomial(&new, P1.degree + P2.degree);

    for (int i = 0; i <= P1.degree; i++) {
        for (int j = 0; j <= P2.degree; j++) {
            new.coef[i + j] += P1.coef[i] * P2.coef[j];
        }
    }
...
Polynomial DerivativePolynomial(Polynomial P){
    ...
    if (P.degree == 0) {
        CreatePolynomial(&new, 0);
        return new;
    }

    CreatePolynomial(&new, P.degree - 1);

    for (int i = 1; i <= P.degree; i++) {
        new.coef[i - 1] = i * P.coef[i];
    }
